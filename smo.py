...
class SVM():
    def __init__(self,trainX,trainY, C=1, kernel=None, difference=1e-3, max_iter=100):
        ...
        self.K = torch.zeros((self.m, self.m)) #核的新特征数组初始化
    ...
    def random_index(self, first_alpha):
        i = torch.Tensor(first_alpha)
        while i == torch.Tensor(first_alpha):
          i = torch.Tensor(np.random.randint(0, self.m - 1))
        return i.int()
    ...
    def predict_row(self, X):
        ...
        return torch.mm((self.alpha * self.Y).T, k_v.T) + self.b
